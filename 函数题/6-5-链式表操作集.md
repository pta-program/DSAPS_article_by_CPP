...
int main()
{
    ...
    tmp = Insert(L, X, P);
    if ( tmp!=ERROR ) cout << "Wrong Answer\n";
    tmp = Delete(L, P);
    if ( tmp!=ERROR ) cout << "Wrong Answer\n";
    for ( P=L; P; P = P->Next ) cout << P->Data << ' ';
    return 0;
}
...
List Insert(List L, ElementType X, Position P) {
    ...
    node->Data = X; node->Next = nullptr;
    if (P == L) { node->Next = L; return node; }
    Position prev = L;
    while (prev && prev->Next != P) prev = prev->Next;
    if (!prev && P != nullptr) { delete node; cout << "Wrong Position for Insertion\n"; return ERROR; }
    if (P == nullptr) { prev = L; if (prev) while (prev->Next) prev = prev->Next; }
    node->Next = P;
...
List Delete(List L, Position P) {
    if (!L || !P) { cout << "Wrong Position for Deletion\n"; return ERROR; }
    if (P == L) { L = L->Next; delete P; return L; }
    Position prev = L;
    while (prev && prev->Next != P) prev = prev->Next;
    if (!prev) { cout << "Wrong Position for Deletion\n"; return ERROR; }
    prev->Next = P->Next; delete P; return L;
}
```
