...
struct DSU {
    ...
    bool unite(int a,int b) { a=find(a); b=find(b); if(a==b)return false; if(p[a]>p[b])swap(a,b); p[a]+=p[b]; p[b]=a; return true; }
};
int main() {
    ios::sync_with_stdio(false); cin.tie(nullptr);
    int n,m; cin >> n >> m; vector<Edge> e(m);
    for(auto& x:e) cin >> x.u >> x.v >> x.w;
    sort(e.begin(),e.end(),[](auto&a,auto&b){return a.w<b.w;});
    DSU dsu(n); long long ans=0; int used=0;
    for(auto x:e) if(dsu.unite(x.u,x.v)) ans+=x.w,++used;
    cout << (used==n-1 ? ans : -1) << '\n';
}
```
