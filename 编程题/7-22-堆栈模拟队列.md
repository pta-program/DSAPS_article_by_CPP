## 7-22 堆栈模拟队列

- **分值：** 25分

## 题目描述

设已知有两个堆栈S1和S2，请用这两个堆栈模拟出一个队列Q。

所谓用堆栈模拟队列，实际上就是通过调用堆栈的下列操作函数:

- `int IsFull(Stack S)`：判断堆栈`S`是否已满，返回1或0；
- `int IsEmpty (Stack S )`：判断堆栈`S`是否为空，返回1或0；
- `void Push(Stack S, ElementType item )`：将元素`item`压入堆栈`S`；
- `ElementType Pop(Stack S )`：删除并返回`S`的栈顶元素。

实现队列的操作，即入队`void AddQ(ElementType item)`和出队`ElementType DeleteQ()`。

## 输入格式

输入首先给出两个正整数`N1`和`N2`，表示堆栈`S1`和`S2`的最大容量。随后给出一系列的队列操作：`A  item`表示将`item`入列（这里假设`item`为整型数字）；`D`表示出队操作；`T`表示输入结束。

## 输出格式

对输入中的每个`D`操作，输出相应出队的数字，或者错误信息`ERROR:Empty`。如果入队操作无法执行，也需要输出`ERROR:Full`。每个输出占1行。

## 输入样例
```
3 2
A 1 A 2 A 3 A 4 A 5 D A 6 D A 7 D A 8 D D D D T
```

## 输出样例
```
ERROR:Full
1
ERROR:Full
2
3
4
7
8
ERROR:Empty
```

## 实现原理与解题思路

将容量较小的栈作为输入栈、容量较大的栈作为输出栈。输入栈未满时直接入队；输入栈已满且输出栈为空时，先整体转移输入栈，再放入新元素，否则报告队列满。出队时优先弹出输出栈，输出栈为空则先转移输入栈。

## 代码流程说明

处理 `A/D/T` 操作。两个栈先按容量重新确定输入栈和输出栈；转移时保持最早入队元素位于输出栈栈顶。每个元素最多被转移一次，摊还时间复杂度 `O(1)`，空间复杂度 `O(N1+N2)`。

## 代码实现

```cpp
// 实现原理：
// 将容量较小的栈作为输入栈、容量较大的栈作为输出栈。输入栈未满时直接入队；输入栈已满且输出栈为空时，先整体转移输入栈，再放入新元素，否则报告队列满。出队时优先弹出输出栈，
// 输出栈为空则先转移输入栈。
// 处理流程：
// 处理 `A/D/T` 操作。两个栈先按容量重新确定输入栈和输出栈；转移时保持最早入队元素位于输出栈栈顶。每个元素最多被转移一次，摊还时间复杂度 `O(1)`，空间复杂度
// `O(N1+N2)`。
#include <algorithm>
#include <iostream>
#include <stack>
using namespace std;
int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);
    int n1, n2;
    cin >> n1 >> n2;
    int inputCap = min(n1, n2);
    stack<int> s1, s2;
    char op;
    while (cin >> op && op != 'T') {
        if (op == 'A') {
            int x;
            cin >> x;
            if ((int)s1.size() < inputCap)
                s1.push(x);
            else if (s2.empty()) {
                while (!s1.empty()) {
                    s2.push(s1.top());
                    s1.pop();
                }
                s1.push(x);
            } else
                cout << "ERROR:Full\n";
        } else if (op == 'D') {
            if (!s2.empty())
                cout << s2.top() << '\n', s2.pop();
            else if (!s1.empty()) {
                while (!s1.empty()) {
                    s2.push(s1.top());
                    s1.pop();
                }
                cout << s2.top() << '\n';
                s2.pop();
            } else
                cout << "ERROR:Empty\n";
        }
    }
}
```

## 代码流程图

```mermaid
flowchart TD
  A[读取操作] --> B{A/D/T?}
  B -- A --> C{输入栈未满?}
  C -- 是 --> D[压入输入栈]
  C -- 否 --> E{输出栈为空?}
  E -- 否 --> F[输出 Full]
  E -- 是 --> G[输入栈转移到输出栈]
  G --> H[新元素压入输入栈]
  B -- D --> I{输出栈为空?}
  I -- 是 --> J[输入栈转移到输出栈]
  I -- 否 --> K[弹出输出栈]
  J --> L{转移后为空?}
  L -- 是 --> M[输出 Empty]
  L -- 否 --> K
  B -- T --> N[结束]
```

## 解题流程图

```mermaid
flowchart LR
  A[入队] --> B[S1]
  B --> C[S2]
  C --> D[出队]
```

## 常见易错点

- 只有 `S2` 为空时才转移 `S1`。
- 满队列判断应考虑两个栈中的元素总数。
- 不要把 `S1` 栈顶直接当成队首。
