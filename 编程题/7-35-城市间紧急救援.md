...
```cpp
// 实现原理：
// 使用 Dijkstra，同时维护最短距离 `dist`、最短路径数 `ways`、最多救援队数 `teams` 和前驱 `pre`。发现更短路径时覆盖三者；距离相等时累加路径数，
// 并保留救援队更多的前驱。
// 处理流程：
// 从起点初始化 `ways=1`、`teams=救援队数`，不断选取最近城市并松弛邻边。结束后沿前驱回溯路径。时间复杂度 `O(n²+m)`，空间复杂度 `O(n+m)`。
#include <algorithm>
...
int main() {
    ...
    const int INF = 1e9;
    vector<int> dis(n, INF), ways(n), res(n), pre(n, -1);
    vector<bool> done(n);
    dis[s] = 0;
    ways[s] = 1;
    res[s] = team[s];
    for (int it = 0; it < n; ++it) {
        ...
        for (auto e : g[v]) {
            int nd = dis[v] + e.w;
            if (nd < dis[e.to])
                dis[e.to] = nd, ways[e.to] = ways[v], res[e.to] = res[v] + team[e.to],
                pre[e.to] = v;
            else if (nd == dis[e.to]) {
                ways[e.to] += ways[v];
                if (res[v] + team[e.to] > res[e.to])
                    res[e.to] = res[v] + team[e.to], pre[e.to] = v;
            }
        }
    }
    vector<int> path;
