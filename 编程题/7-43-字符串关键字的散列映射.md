...
#include <string>
using namespace std;
int main(){ios::sync_with_stdio(false);cin.tie(nullptr);int n,p;cin>>n>>p;vector<bool>used(p);unordered_map<string,int>position;for(int z=0;z<n;++z){string s;cin>>s;auto it=position.find(s);int pos;if(it!=position.end())pos=it->second;else{long long h=0;int start=max(0,(int)s.size()-3);for(int i=start;i<(int)s.size();++i)h=h*32+s[i]-'A';pos=(int)(h%p);if(used[pos])for(int step=1;step<=p;++step){long long d=1LL*step*step;long long cand=(h+d)%p;if(!used[cand]){pos=(int)cand;break;}cand=(h-d)%p;if(cand<0)cand+=p;if(!used[cand]){pos=(int)cand;break;}}used[pos]=true;position[s]=pos;}if(z)cout<<' ';cout<<pos;}cout<<'\n';}
```
