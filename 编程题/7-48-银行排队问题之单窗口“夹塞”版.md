...
#include <algorithm>
using namespace std;
struct Customer { string name; int arrive, service, group = 0; };
...
int main() {
    ...
    for (int g = 1; g <= m; ++g) {
        int k; cin >> k;
        while (k--) { string name; cin >> name; group[name] = g; }
    }
    vector<Customer> a(n);
    for (auto& x : a) {
        cin >> x.name >> x.arrive >> x.service;
        x.service = min(x.service, 60);
        if (group.count(x.name)) x.group = group[x.name];
    }
