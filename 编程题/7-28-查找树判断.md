...
```cpp
// 实现原理：
// 递归验证一段序列是否能成为 BST 或镜像 BST 的前序序列。首元素为根；普通 BST 中先找连续的小于根的左子树段，剩余部分必须都大于等于根；镜像情况将比较方向反过来。
// 验证通过后按同样分段递归收集后序序列。
// 处理流程：
// 对普通规则和镜像规则各尝试一次，任一成功即输出 `YES` 及后序结果。每个区间中的元素被扫描一次，时间复杂度最坏 `O(n²)`，递归空间 `O(n)`。
#include <iostream>
...
bool build(int l, int r, bool mirror) {
    ...
    if (!mirror)
        ...
        while (p <= r && a[p] >= root)
            ++p;
    int split = p;
    while (p <= r && (!mirror ? a[p] >= root : a[p] < root))
        ++p;
    if (p <= r)
        return false;
    if (!build(l + 1, split - 1, mirror) || !build(split, r, mirror))
        return false;
    post.push_back(root);
    return true;
}
