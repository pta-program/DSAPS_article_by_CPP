...
int main(){ios::sync_with_stdio(false);cin.tie(nullptr);vector<int>a=read(),b=read(),c;size_t i=0,j=0;while(i<a.size()&&j<b.size()){if(a[i]==b[j])c.push_back(a[i]),++i,++j;else if(a[i]<b[j])++i;else ++j;}if(c.empty())cout<<"NULL";else for(size_t k=0;k<c.size();++k){if(k)cout<<' ';cout<<c[k];}cout<<'\n';}
```
