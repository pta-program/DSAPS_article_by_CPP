...
#include <algorithm>
using namespace std;
int main(){ios::sync_with_stdio(false);cin.tie(nullptr);int n,k;cin>>n>>k;vector<vector<string>>course(k+1);for(int i=0;i<n;++i){string name;int c;cin>>name>>c;while(c--){int x;cin>>x;course[x].push_back(name);}}for(int i=1;i<=k;++i){sort(course[i].begin(),course[i].end());cout<<i<<' '<<course[i].size()<<'\n';for(auto&s:course[i])cout<<s<<'\n';}}
```
